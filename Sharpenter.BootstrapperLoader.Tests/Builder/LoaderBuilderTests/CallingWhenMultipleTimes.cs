...
namespace Sharpenter.BootstrapperLoader.Tests.Builder.LoaderBuilderTests
{
    public class CallingWhenMultipleTimes
    {
        ...
        {
            Assert.Equal(new[] { "Configure1", "Configure2", "Configure" }, _loader.Config.ConfigureMethods.Keys.ToArray());
            Assert.Equal(new[] { "ConfigureContainer1", "ConfigureContainer2", "ConfigureContainer" }, _loader.Config.ConfigureContainerMethods.Keys.ToArray());
        }
    }
}